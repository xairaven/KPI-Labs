...
hill_climb(_-state(OldBoat, Wolf, Goat, Cabbage), History, [Move | Moves]):-
    ...
    get_possible_moves(state(OldBoat, Wolf, Goat, Cabbage), Boat, PossibleMoves),
...
get_possible_moves(State, Boat, PossibleMoves) :-
    setof(HeuristicValue-state(Boat, WolfTemp, GoatTemp, CabbageTemp)-MoveDescription, 
    (move(State, state(Boat, WolfTemp, GoatTemp, CabbageTemp), MoveDescription),
