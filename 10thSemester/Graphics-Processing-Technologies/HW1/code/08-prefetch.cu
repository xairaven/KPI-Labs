...
int main(void)
{
    ...
    // Explicitly define the target memory location
    cudaMemLocation deviceLocation;
    deviceLocation.type = cudaMemLocationTypeDevice;
    deviceLocation.id = 0;

    // Prefetch the data using the new struct
    cudaMemPrefetchAsync(x, N * sizeof(float), deviceLocation, 0);
    cudaMemPrefetchAsync(y, N * sizeof(float), deviceLocation, 0);
