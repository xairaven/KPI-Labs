...
use crate::interpolation::examples::Functions;
use crate::interpolation::ui::input::InputModal;
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            Grid::new("Approximation.Settings")
                ...
                .show(ui, |ui| {
                    ...
                    if state.interpolation_context.is_initialized() {
                        ...
                        ui.end_row();
                        ui.checkbox(&mut state.is_linear_enabled, "Enable Linear Curve");
                        ui.end_row();
                        ui.checkbox(
                            &mut state.is_lagrange_enabled,
                            "Enable Lagrange Curve",
                        );
                        ui.end_row();
                        ...
                        ui.label("Lagrange Curve Color: ");
                        egui::color_picker::color_edit_button_srgba(
                            ui,
                            &mut state.lagrange_color,
                            egui::color_picker::Alpha::Opaque,
                        );
                        ui.end_row();
                    }
            ...
            ui.vertical_centered_justified(|ui| {
                if ui.button("Reset Initialization").clicked() {
                    state.interpolation_context = Default::default();
                }
            });
            ...
            ui.vertical_centered_justified(|ui| {
                if ui.button("Load From...").clicked() {
                    if let Err(err) = interpolation::io::load_with_file_pick(
                        &mut state.interpolation_context,
                    ) {
                ...
                {
                    if let Err(err) = interpolation::io::save_with_file_pick(
                        &mut state.interpolation_context,
                    ) {
                        let modal = Box::new(MessageModal::error(&err.to_string()));
                        let _ = state.modals_tx.try_send(modal);
            ...
            ui.vertical_centered_justified(|ui| {
                ...
                for function in Functions::vec() {
                    if ui.button(function.name()).clicked() {
                        state.interpolation_context.points_view = function.points();
                        state.interpolation_context.initialize();
                    }
                }
            });
