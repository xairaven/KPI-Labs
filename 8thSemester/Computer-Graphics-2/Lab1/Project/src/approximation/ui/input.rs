...
impl Modal for InputModal {
    fn show(&mut self, main_state: &mut State, ui: &egui::Ui) {
        let state = &mut main_state.approximation_state;
        ...
        if self.is_open {
            let modal = egui::Modal::new(Id::new(self.id)).show(ui.ctx(), |ui| {
                ...
                if state.points_view.is_empty() {
                    state.points_view.push(Point2D::default());
                }
                ...
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        ...
                        Grid::new("Approximation.Points")
                            ...
                            .show(ui, |ui| {
                                for (index, point) in
                                    state.points_view.iter_mut().enumerate()
                                {
                        ...
                        if let Some(index) = to_remove_element {
                            state.points_view.remove(index);
                        }
                    });
                        ...
                        if ui.button("Save").clicked() {
                            if let Err(err) = state.initialize() {
                                *state = Default::default();
                                let modal =
                                    Box::new(MessageModal::error(&err.to_string()));
                                let _ = main_state.modals_tx.try_send(modal);
                            }
                            self.close(state);
                        }
                    });
...
impl InputModal {
    fn close(&mut self, state: &mut ApproximationState) {
